...
import { Plus, Tag, X, Check, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function AdminCouponsPage() {
    ...
    const handleAdd = () => {
        ...
        setCoupons(prev => [...prev, { coupon_code: form.code.toUpperCase(), discount: Number(form.discount), expiration_date: form.expiration_date, min_order_amount: Number(form.min_order_amount) || 0, usage_limit: Number(form.usage_limit) || 100, used_count: 0, is_active: form.is_active }]);
        setShowForm(false); setForm({ code: '', discount: '', expiration_date: '', min_order_amount: '', usage_limit: '100', is_active: true });
    };

    const toggleActive = (code) => setCoupons(prev => prev.map(c => c.coupon_code === code ? { ...c, is_active: !c.is_active } : c));
    const deleteCoupon = (code) => setCoupons(prev => prev.filter(c => c.coupon_code !== code));
                    ...
                    <tbody>
                        {coupons.map(c => (
                            <tr key={c.coupon_code}>
                                ...
                                <td>
                                    <button onClick={() => toggleActive(c.coupon_code)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <div className="modal-overlay" onClick={() => setShowForm(false)}>
                    <div className="modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="modal-footer">
                            ...
                            <button id="save-coupon-btn" className="btn btn-primary" onClick={handleAdd}><Check size={14} /> Create Coupon</button>
                        </div>
                    </div>
                </div>
            )}
